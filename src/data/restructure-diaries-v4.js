 ...
 *
 * v4 改进：
 * - 修复"观察者视角"section被当成事件的问题（它是观察类）
 * - 优化跨午夜时间处理：凌晨0-6点事件排在当天事件最后（作为前一天的深夜）
 * - 改进事件拆分："约 XX:XX" 格式的行应作为独立事件
 * - 更好地处理观察与事件的匹配
 */
 ...
 * 解析时间，返回分钟数
 * 特殊处理：
 * - 凌晨 0:00 - 6:00 的事件：加 1440（24*60），排在当天最后（作为前一天深夜延续）
 * - 24:00+ 保持原样
 */
function parseTime(text) {
  ...
  if (match) {
    ...
    if (hour >= 24) val = hour * 60 + minute; // 24+ 保持原样
    return val;
  }
  ...
  if (match) {
    ...
    let minute = parseInt(match[2], 10);
    let val = hour * 60 + minute;
    if (hour > 0 && hour < 6) val += 1440; // 凌晨0-6点排最后（作为前一天深夜）
    if (hour >= 24) val = hour * 60 + minute;
    return val;
  }
...
function extractEventsFromSection(sectionContent, sectionTitle) {
  ...
  for (let rawLine of lines) {
    ...
    // 条件4：行首是"约"+数字（约15:00这种）
    if (!isNewEvent && /^约\s*\d{1,2}[点:：]/.test(line)) {
      isNewEvent = true;
    }

    // 条件5：行首是模糊时间词（但排除"XX与YY"的连接词情况）
    if (!isNewEvent) {
      const timeKwMatch = line.match(/^(凌晨|清晨|晨起|早上|上午|中午|午后|下午|傍晚|晚上|夜间|深夜|半夜|晨读|午饭|晚饭|晚餐|夜间|深夜)/);
      if (timeKwMatch) {
    ...
    if (isNewEvent) {
      ...
      if (line.startsWith('◦') || line.startsWith('  ◦')) {
        // 这是观察，应该被上层处理，不放在事件内容里
        // 但为了简化，我们先保留，观察提取会在后面做
  ...
  for (let rawLine of lines) {
    ...
    if (!isNewObs && hasColon && colonIdx > 0 && colonIdx < 30) {
      ...
      if (!/[，。；,.;]/.test(beforeColon) && beforeColon.length > 1) {
        // 还要排除"XX与YY："这种不是观察标题的情况
        if (!/[与和及跟]/.test(beforeColon) || beforeColon.length > 6) {
          isNewObs = true;
        }
      }
    }
          ...
          for (const obsLine of embeddedObs) {
            ...
            if (cIdx > 0) {
              ...
              if (colonIdx > 0 && colonIdx < 30) {
                obsTitle = cleanObs.substring(0, colonIdx).trim();
...
// 抽样展示
const sampleDates = ['2026-04-26', '2026-04-27', '2026-07-01', '2026-07-02'];
